...
def calculate_peak_to_valley(
    ...
    lower_counts, lower_unc = integrate_energy_window(spectrum, lower_min_energy, lower_max_energy)
    upper_counts, upper_unc = integrate_energy_window(spectrum, upper_min_energy, upper_max_energy)
    ...
    if valley_counts <= 0:
        raise ValueError(
            "The valley discontinuity is not positive. "
            "Move the lower/upper valley regions or check the selected peak."
        )
    if peak.area <= 0:
